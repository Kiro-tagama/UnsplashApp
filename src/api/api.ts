...
const key:string = APP_KEY
const url:string =  "https://api.unsplash.com/"

async function apiAll() {
  return await fetch(url+"photos?per_page=30&client_id="+key)
  .then(res=>res.json())
} 
...
async function apiSearch(params:String) {
  return await fetch(url+"search/photos?query="+params+"&client_id="+key)
  .then(res=>res.json())
} // ok
